...
use egui::{
    ...
    epaint::TextShape, lerp, pos2, vec2,
};
...
impl<Tab> DockArea<'_, Tab> {
    ...
    fn tabs(
        ...
        for tab_index in 0..tabs_len {
            ...
            // MN-PATCH (round 28): CSP shows the close × on the ACTIVE tab
            // only — a × on every tab reads as a row of stray buttons once
            // titles ellipsize (owner report: "two ×s side by side"). The
            // hovered tab also reveals its × so background tabs stay
            // directly closable; last frame's hover, read before layout.
            let tab_hovered_last = tabs_ui
                .ctx()
                .read_response(id)
                .is_some_and(|r| r.hovered());
            let show_close_button =
                self.show_close_buttons && closeable && (is_active || tab_hovered_last);
    ...
    fn try_initiate_tab_drag(
        ...
        // MN-PATCH (MangaNakama, round 22): 30px horizontal / 6px vertical
        // feels like the tab is stuck on compact palette strips; 8px matches
        // egui's own drag threshold.
        (delta.x.abs() > 8.0 || delta.y.abs() > 8.0).then(|| {
            ui.memory_mut(|mem| {
                mem.data.insert_temp(
                    // MN-PATCH: global key + owner id (see show/mod.rs).
                    super::dnd_drag_key(),
                    Some(DragData {
    ...
    fn tab_title(
        ...
        // MN-PATCH (MangaNakama, 2026-08-16): ellipsize the title to the
        // space the strip actually gives the tab. Upstream lays the FULL
        // galley (f32::INFINITY wrap width) centered in `text_rect`, so
        // whenever `preferred_width` squeezes the tab (fill_tab_bar, narrow
        // dock columns) the title sticks out over the close button and the
        // neighbouring tab. Truncating to the text area makes squeezed tabs
        // read like every desktop tab bar: "Tool Prope…".
        let galley = if let Some(pw) = preferred_width {
            let avail = (pw - close_button_size - 2.0 * x_spacing).max(0.0);
            if avail > 0.0 {
                label.into_galley(ui, Some(TextWrapMode::Truncate), avail, TextStyle::Button)
            } else {
                label.into_galley(ui, Some(TextWrapMode::Truncate), 0.0, TextStyle::Button)
            }
        } else {
            label.into_galley(ui, None, f32::INFINITY, TextStyle::Button)
        };
    ...
    fn tab_body(
        ...
        if let Some(pointer) = state.last_hover_pos {
            ...
            // MN-PATCH (MangaNakama, round 22): publish hover during ANY
            // in-flight dock drag, not just one started by THIS DockArea —
            // the global drag payload may be owned by the sibling DockArea,
            // and the owner reads the hover key to resolve cross-column
            // drops. `drag_start` stays per-area (only the owner sets it).
            let any_dock_drag = state.drag_start.is_some()
                || ui.ctx().memory(|mem| {
                    mem.data
                        .get_temp::<Option<DragData>>(super::dnd_drag_key())
                        .is_some()
                });

            // Use rect.contains instead of response.hovered as the dragged tab covers
            // the underlying responses.
            if any_dock_drag && rect.contains(pointer) && is_dragged_valid {
                ...
                ui.memory_mut(|mem| {
                    ...
                    mem.data.insert_temp(
                        super::dnd_hover_key(),
                        Some(HoverData { rect, dst, tab, owner: self.id }),
                    );
                });
            }
        }
