...
pub(super) struct HoverData {
    ...
    /// MN-PATCH (MangaNakama, round 22): which DockArea published this
    /// hover. Node/surface indices are only meaningful inside that area's
    /// own DockState — a foreign hover must never be indexed by another
    /// tree (the owner treats it as "no hover" and falls back to tear-off).
    pub owner: egui::Id,
}
...
pub(super) struct DragData {
    ...
    pub rect: Rect,
    /// MN-PATCH (MangaNakama, round 22): which DockArea started this drag.
    /// DnD keys are global (see show/mod.rs), so sibling DockAreas with
    /// separate DockStates all see the payload; only the owner may resolve
    /// the drop. Surface indices alone cannot tell the trees apart (both
    /// have a main surface 0).
    pub owner: egui::Id,
}
