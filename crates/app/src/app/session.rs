                    ...
                    match (&e.bytes, e.blank) {
                        ...
                        (None, Some((w, h, n))) => {
                            ...
                            if self.seed_frame_folder
                                && let Some(p) = self.page.as_ref().filter(|p| p.has_guides())
                            {
                                // The same seeding blank_page_doc_at does —
                                // DocSession is not an App, so the frame
                                // folder build is inlined (keep the two in
                                // sync; that fn is the reference).
...
impl App {
    ...
    fn install(&mut self, s: DocSession) {
        ...
        self.print_crop_marks = s.print_crop_marks;
        self.cover = s.cover;
