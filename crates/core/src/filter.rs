...
//!
//! The kernels themselves live beside this file — `blur.rs` (the box/Gaussian
//! family, motion, radial, spin, unsharp), `distort.rs` (FL-020..023) and
//! `lines.rs` (LC-001 dust, LC-002 line width). What stays here is the enum,
//! its menu seeds, the dispatch (`reach`/`run`/`is_identity`) and the
//! gather/scatter every one of them rides.
//!
...
impl Filter {
    ...
    pub fn reach(self) -> i32 {
        match self {
            ...
            // circle by construction ([`distort::radial_frame`]), so like the two
...
/// One integer symmetric convolution pass, as [`blur::box_h`] and [`blur::tent_h`]
...
pub(super) fn sample_bilinear(src: &Raster, fx: f32, fy: f32) -> [f32; TILE_CHANNELS] {
    let x0 = fx.floor();
    let y0 = fy.floor();
    let tx = fx - x0;
    let ty = fy - y0;
